import Link from "next/link";

export default function Test() {
  return (
    <>
      <h1>App</h1>
      <ul>
        {Object.entries(URLS).map(([name, query]) => (
          <li key={name}>
            <Link href={`/?data=${query}`}>{name}</Link>
          </li>
        ))}
      </ul>
      <h1>Pages</h1>
      <ul>
        {Object.entries(URLS).map(([name, query]) => (
          <li key={name}>
            <Link href={`/headless?data=${query}`}>{name}</Link>
          </li>
        ))}
      </ul>
      <h1>Images</h1>
      <ul>
        {Object.entries(URLS).map(([name, query]) => (
          <li key={name}>
            <Link href={`/api/ogimage/?data=${query}`}>{name}</Link>
          </li>
        ))}
      </ul>
    </>
  );
}

const URLS = {
  default: ``,
  days: ``,
  weeks: ``,
  months: ``,
  years: ``,
};
